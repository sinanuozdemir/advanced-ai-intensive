"""OpenRouter-backed LLM factory used across every Week 1 notebook.

One key (`OPENROUTER_API_KEY`), one client pattern (`ChatOpenAI` pointing at the
OpenRouter base URL), every model. Notebooks ask for a model by *role*
(`get_llm("cheap_workhorse")`) or by raw OpenRouter slug
(`get_llm("anthropic/claude-sonnet-4.5")`).
