...
> **Course note (week 2 SDR app).** This server is **vendored** into the repo
> and built as the `email-mcp-server` service in `apps/sdr_multi_agent/docker-compose.yml`.
> You do **not** need to clone the upstream repo or run `npm install` by hand.
> Set `RESEND_API_KEY`, `SENDER_EMAIL_ADDRESS`, and (optionally)
> `REPLY_TO_EMAIL_ADDRESSES` in your `.env` and `docker compose up` will
> launch it automatically. The rest of this README is the upstream Resend
> README, kept for reference if you want to use this server with Cursor or
