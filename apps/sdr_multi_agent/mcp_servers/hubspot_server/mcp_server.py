#!/usr/bin/env python3
"""
HubSpot MCP Server for contact management.
Provides tools to create, fetch, update, and annotate contacts in HubSpot CRM:
create_contact, fetch_contacts, update_contact, attach_note_to_contact,
...
async def handle_call_tool(name: str, arguments: Dict[str, Any]) -> List[types.TextContent]:
    ...
    if name == "create_contact":
        ...
        try:
            ...
            # NOTE: HubSpot contacts do NOT have a generic "notes" property.
            # Attempting to PATCH it returns 400 INVALID_PROPERTY and the whole
            # update (including hs_lead_status) is rolled back. Notes are
            # stored as engagement objects via attach_note_to_contact instead.
            # We silently drop the field here so lead_status still applies.
            note_to_attach = notes if (notes is not None and str(notes).strip()) else None
            ...
            async with httpx.AsyncClient() as client:
                ...
                if response.status_code == 200:
                    ...
                    # Best-effort: if the agent passed a note, attach it as a
                    # proper engagement object (HubSpot rejects "notes" on
                    # contacts directly; engagement is the supported path).
                    if note_to_attach:
                        try:
                            import time as _time
                            note_data = {
                                "properties": {
                                    "hs_note_body": note_to_attach,
                            ...
                            if note_resp.status_code == 201:
                                result += f"\n📝 Note attached as engagement (id={note_resp.json().get('id')})."
                            else:
                                print(
                                    f"[hubspot] note attach failed for {contact_id}: "
                                    f"{note_resp.status_code} {note_resp.text[:500]}",
                                    flush=True,
                                )
                                result += f"\n⚠️ Could not attach note ({note_resp.status_code})."
                        except Exception as _exc:  # noqa: BLE001
                            print(f"[hubspot] note attach error: {_exc}", flush=True)
                            result += f"\n⚠️ Note attach errored: {_exc}"
                    ...
                    # Other error — surface the full HubSpot payload so the
                    # agent (and the worker log) can see exactly which property
                    # the API rejected.
                    raw_body = ""
                    try:
                    ...
                    error_message = error_details.get('message', f'HTTP {response.status_code}')
                    print(
                        f"[hubspot] update_contact {contact_id} failed: "
                        f"{response.status_code} {raw_body[:1000]}",
                        flush=True,
                    )
